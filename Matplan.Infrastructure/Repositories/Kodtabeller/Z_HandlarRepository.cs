...
public class Z_HandlarRepository : RepositoryBase<Z_Handlar>, IZ_HandlarRepository
{
    public Z_HandlarRepository(MatplanDBContext context) : base(context)
    {
    }

    public async Task<Z_Handlar?> GetZ_HandlarAsync(byte id, bool trackChanges = false)
    {
        var retur = await FindByCondition(c => c.Storedpfu.Equals(id), trackChanges).FirstOrDefaultAsync();
//        if (retur == null) throw new Z_HandlarNotFoundException(id);
        return retur;
    }


    public async Task<IPagedList<Z_Handlar>> GetZ_HandlarListAsync(KodtabellByteRequestParams requestParams, bool trackChanges = false)
    {
        var query = FindAll(trackChanges);
        query = query.OrderBy(o => o.Storedpfu);

        var retur = await PagedList<Z_Handlar>.CreateAsync(query, requestParams.PageNumber, requestParams.PageSize);
        return (IPagedList<Z_Handlar>)retur;
    }


    public async Task<bool> Z_HandlarExistAsync(byte id)
    {
        return await Context.Z_Handlar.AnyAsync(c => c.Storedpfu == id);
    }

    //public new Z_Handlar Insert(Z_Handlar Z_Handlar)
    //{
    //    base.Insert(Z_Handlar);
    //    return Z_Handlar;
    //}

    //public new Z_Handlar Update(Z_Handlar Z_Handlar)
    //{
    //    base.Update(Z_Handlar);
    //    return Z_Handlar;
    //}

    //public override void Delete(Z_Handlar Z_Handlar)
    //{
    //    base.Delete(Z_Handlar);
    //}
